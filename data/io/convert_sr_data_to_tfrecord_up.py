...
def read_xml_gtbox_and_label(xml_path):
    ...
    for child_of_root in root:
        ...
        if child_of_root.tag == 'size':
            ...
            if img_width != RE_LEN:
                w_ratio = img_width/RE_LEN
                img_width = RE_LEN
            if img_height != RE_LEN:
                h_ratio = img_height/RE_LEN
                img_height = RE_LEN
...
def convert_pascal_to_tfrecord():
    ...
    # low_image_path = FLAGS.VOC_dir + FLAGS.low_image_dir
    ...
    for count, xml in enumerate(glob.glob(xml_path + '/*.xml')):
        ...
        #　low_img_path = low_image_path + '/' + img_name
        ...
        img = cv2.resize(cv2.imread(img_path), (RE_LEN, RE_LEN))
        low_img = cv2.resize(img, (int(RE_LEN / SCALE), int(RE_LEN / SCALE)))
        print(low_img.shape)
        low_img_height, low_img_width, _ = low_img.shape
        feature = tf.train.Features(feature={
            ...
            'low_img_height': _int64_feature(low_img_height),
            'low_img_width': _int64_feature(low_img_width),
            'low_img': _bytes_feature(low_img.tostring()),
