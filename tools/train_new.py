...
# os.environ["CUDA_VISIBLE_DEVICES"] = '0'#cfgs.GPU_GROUP


def train():

    faster_rcnn = build_whole_network_new.DetectionNetwork(base_network_name=cfgs.NET_NAME,
                                                           is_training=True)
    ...
    with tf.name_scope('get_batch'):
        ...
        if cfgs.USE_PAN:
            gtboxes_binary_map = binary_map_generate(img_batch, tf.squeeze(gtboxes_and_label_batch, 0))
    ...
    with slim.arg_scope([slim.conv2d, slim.conv2d_in_plane,
        ...
        if cfgs.USE_PAN:
            ...
            final_boxes_r, final_scores_r, final_category_r, loss_dict = faster_rcnn.build_whole_detection_network(
                ...
                gtboxes_binary_map=gtboxes_binary_map)
        else:
            final_boxes_h, final_scores_h, final_category_h, \
            final_boxes_r, final_scores_r, final_category_r, loss_dict = faster_rcnn.build_whole_detection_network(
                input_img_batch=img_batch,
    ...
    if cfgs.USE_PAN:
        pan_loss = loss_dict['pan_loss']
        tf.summary.scalar('PAN_LOSS', pan_loss)
        total_loss = rpn_total_loss + fastrcnn_total_loss + weight_decay_loss + pan_loss
    else:
