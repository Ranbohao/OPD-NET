...
def draw_box_with_color_rotate(img_batch, boxes, text):
    ...
    def draw_box_cv(img, boxes, text):
        ...
        for box in boxes:
            ...
            rect_tmp = [rect[0], rect[3], rect[2], rect[1]]
            rect = np.int0(rect_tmp)
            color = (np.random.randint(255), np.random.randint(255), np.random.randint(255))
            ...
            if cfgs.DETECT_HEAD:
                head_x = (rect[1][0] + rect[2][0])/2
                head_x = int(head_x)
                head_y = (rect[1][1] + rect[2][1])/2
                head_y = int(head_y)
                cv2.circle(img, (head_x, head_y), int(h/2), color, 2)
        ...
        for i, box in enumerate(boxes):
            ...
            if label != 0:
                ...
                cv2.putText(img,
                            ...
                            color=(color[1], color[2], color[0]))
                if cfgs.DETECT_HEAD:
                    head_x = (rect[2][0] + rect[3][0])/2
                    head_x = int(head_x)
                    head_y = (rect[2][1] + rect[3][1])/2
                    head_y = int(head_y)
                    cv2.circle(img, (head_x, head_y), int(h/2), color, 2)
        cv2.putText(img,
                    text=str(num_of_object),
