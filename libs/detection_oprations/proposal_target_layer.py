...
def proposal_target_layer(rpn_rois, gt_boxes_h, gt_boxes_r):
    ...
    bbox_targets_r = bbox_targets_r.reshape(-1, (cfgs.CLASS_NUM+1) * cfgs.RBB_LEN)
...
def _get_bbox_regression_labels_r(bbox_target_data, num_classes):
    ...
    clss = bbox_target_data[:, 0]
    bbox_targets = np.zeros((clss.size, cfgs.RBB_LEN * num_classes), dtype=np.float32)
    inds = np.where(clss > 0)[0]
    for ind in inds:
        cls = clss[ind]
        start = int(cfgs.RBB_LEN * cls)
        end = start + cfgs.RBB_LEN
        bbox_targets[ind, start:end] = bbox_target_data[ind, 1:]
