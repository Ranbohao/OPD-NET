...
if cfgs.USE_CPCRM:
    from libs.box_utils import encode_and_decode_cpcrm as encode_and_decode
else:
...
if cfgs.USE_PAN:
    from libs.networks.pan import build_pan
    from libs.networks.pan.binary_map import binary_map_generate
    from libs.networks.pan import show_feature
...
class DetectionNetwork(object):
    ...
    def __init__(self, base_network_name, is_training):
        ...
        self.input_img_batch = None

    def build_base_network(self, input_img_batch):

        if self.base_network_name.startswith('resnet_v1'):
            if cfgs.USE_FEATURE_BALANCE:
                return resnet.resnet_base_balance(input_img_batch, scope_name=self.base_network_name, is_training=self.is_training)
            else:
    ...
    def postprocess_fastrcnn_r(self, rois, bbox_ppred, scores, img_shape):
        ...
        with tf.name_scope('postprocess_fastrcnn_r'):
            rois = tf.stop_gradient(rois)
            scores = tf.stop_gradient(scores)
            bbox_ppred = tf.reshape(bbox_ppred, [-1, cfgs.CLASS_NUM + 1, cfgs.RBB_LEN])
            bbox_ppred = tf.stop_gradient(bbox_ppred)
    ...
    def roi_pooling(self, feature_maps, rois, img_shape):
        ...
        def _separable_conv_layer(bottom, filters, kernel_size, strides, name=None, dilation_rate=1, activation=None):
            conv = tf.layers.separable_conv2d(
                ...
                padding='same',
                name=name,
                data_format='channels_last',#self.data_format,
                use_bias=False,
        ...
        with tf.variable_scope('ROI_Warping'):
            ...
            if cfgs.USE_LIGHT_HEAD:
                state5_conv1_1 = slim.separable_conv2d(inputs=feature_maps, num_outputs=256, kernel_size=[1, 15], stride=1,
                                                       activation_fn=tf.nn.relu, normalizer_fn=slim.batch_norm, depth_multiplier=1, 
                                                       trainable=self.is_training, scope='state5_conv1_1')
                state5_conv1_2 = slim.separable_conv2d(inputs=state5_conv1_1, num_outputs=cfgs.LIGHT_CHANNEL, kernel_size=[15, 1], stride=1,
                                                       activation_fn=tf.nn.relu, normalizer_fn=slim.batch_norm, depth_multiplier=1,
                                                       trainable=self.is_training, scope='state5_conv1_2')
                state5_conv2_1 = slim.separable_conv2d(inputs=feature_maps, num_outputs=256, kernel_size=[1, 15], stride=1,
                                                       activation_fn=tf.nn.relu, normalizer_fn=slim.batch_norm, depth_multiplier=1,
                                                       trainable=self.is_training, scope='state5_conv2_1')
                state5_conv2_2 = slim.separable_conv2d(inputs=state5_conv2_1, num_outputs=cfgs.LIGHT_CHANNEL, kernel_size=[15, 1], stride=1,
                                                       activation_fn=tf.nn.relu, normalizer_fn=slim.batch_norm, depth_multiplier=1,
                                                       trainable=self.is_training, scope='state5_conv2_2')
                feature_maps = state5_conv1_2 + state5_conv2_2
        ...
        with tf.variable_scope('Fast-RCNN'):
            ...
            if self.base_network_name.startswith('resnet'):
                if cfgs.USE_LIGHT_HEAD:
                    ...
                    fc_flatten = slim.fully_connected(fc_flatten,
                                                      num_outputs=2048,
                                                      weights_initializer=cfgs.INITIALIZER,
                                                      activation_fn=tf.nn.relu, trainable=self.is_training,
    ...
    def build_loss(self, rpn_box_pred, rpn_bbox_targets, rpn_cls_score, rpn_labels,
        ...
        :param bbox_pred_r: [-1, cfgs.RBB_LEN*(cls_num+1)]
        :param bbox_targets_r: [-1, cfgs.RBB_LEN*(cls_num+1)]
    ...
    def build_whole_detection_network(self, input_img_batch, gtboxes_r_batch, gtboxes_h_batch,
        ...
        if cfgs.USE_PAN:
            pan = build_pan.PAN(base_network_name=cfgs.NET_NAME, is_training=self.is_training,
                            is_only_pan=False)
            if self.is_training:
                ...
                saliency_mask = pan.build_pan_network(inputs=feature_to_cropped)
        ...
        if self.is_training:
            ...
            with tf.control_dependencies([rpn_labels]):
                with tf.variable_scope('sample_RCNN_minibatch'):
                    ...
                    bbox_targets_r = tf.reshape(bbox_targets_r, [-1, cfgs.RBB_LEN*(cfgs.CLASS_NUM+1)])
        ...
        if checkpoint_path != None:
            if cfgs.RESTORE_FROM_RPN:
                ...
                for var in global_variables:
                    if 'Fast' in var.op.name and 'moving' not in var.op.name:
                        var_name_in_ckpt = var.op.name + '/Momentum'
                        nameInCkpt_Var_dict[var_name_in_ckpt] = var
                        print(var_name_in_ckpt)
                    else:
                        
                        nameInCkpt_Var_dict[var.op.name] = var
                        print(var.op.name)
                '''
