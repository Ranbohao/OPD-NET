...
def smooth_l1_loss_rcnn_r(bbox_pred, bbox_targets, label, num_classes, sigma=1.0):
    ...
    :param bbox_pred: [-1, (cfgs.CLS_NUM +1) * cfgs.RBB_LEN]
    :param bbox_targets:[-1, (cfgs.CLS_NUM +1) * cfgs.RBB_LEN]
    :param label:[-1]
    :param num_classes:
    :param sigma:
    :return:
    ...
    bbox_pred = tf.reshape(bbox_pred, [-1, num_classes, cfgs.RBB_LEN])
    bbox_targets = tf.reshape(bbox_targets, [-1, num_classes, cfgs.RBB_LEN])
